...
contract TvlAdapterVeCrv {
  ...
  address public yveCrvDaoAddress = 0xc5bDdf9843308380375a611c18B50Fb9341f502A; // veCRV-DAO "Vault"
  ...
  function adapterInfo() public view returns (AdapterInfo memory) {
    return
      AdapterInfo({id: address(this), typeId: "VE_CRV", categoryId: "SPECIAL"});
  }
  ...
  function assetTvlUsdc(address assetAddress) public view returns (uint256) {
    ...
    uint256 adjustedBalanceUsdc = IOracle(
    ...
    returns (AssetTvlBreakdown memory)
  {
    address tokenAddress = underlyingTokenAddress(assetAddress);
    uint256 underlyingBalanceAmount = assetBalance(assetAddress);
    uint256 tokenPriceUsdc = IOracle(0x190c2CFC69E68A8e8D5e2b9e2B9Cc3332CafF77B)
    ...
    return
      AssetTvlBreakdown({
        ...
        adjustedBalance: underlyingBalanceAmount - delegatedBalanceAmount,
        adjustedBalanceUsdc: IOracle(0x190c2CFC69E68A8e8D5e2b9e2B9Cc3332CafF77B)
    ...
    returns (AssetTvlBreakdown[] memory)
  {
    ...
    tvlData[0] = assetTvlBreakdown(assetsAddresses()[0]);
    return tvlData;
  }
