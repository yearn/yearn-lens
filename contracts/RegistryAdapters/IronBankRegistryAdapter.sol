...
interface IUnitroller {
  ...
  function markets(address marketAddress) external view returns (Market memory);
}
...
contract RegistryAdapterIronBank is Ownable {
  ...
  function addressesGenerator() public view returns (IAddressesGenerator) {
    return
      IAddressesGenerator(
        yearnAddressesProvider().addressById("ADDRESSES_GENERATOR_IRON_BANK")
      );
  }
  ...
  {
    bool enteredMarket;
    address[] memory markets = IUnitroller(comptrollerAddress()).getAssetsIn(
      accountAddress
    );
  ...
  function assetsUserMetadata(
    ...
    uint256 numberOfAssets = _assetsAddresses.length;
    AssetUserMetadata[] memory _assetsUserMetadata = new AssetUserMetadata[](
      numberOfAssets
    );
    for (uint256 assetIdx = 0; assetIdx < numberOfAssets; assetIdx++) {
      address assetAddress = _assetsAddresses[assetIdx];
  ...
  function assetUnderlyingTokenPriceUsdc(address assetAddress)
    ...
    returns (uint256)
  {
    address _underlyingTokenAddress = assetUnderlyingTokenAddress(assetAddress);
    ...
    returns (AssetDynamic memory)
  {
    ...
    AssetMetadata memory metadata = AssetMetadata({
      ...
      borrowApyBips: (asset.borrowRatePerBlock() * blocksPerYear) / 10**14,
      liquidity: liquidity,
    ...
    returns (AdapterPosition memory)
  {
    ...
    if (borrowLimitUsdc > 0) {
      utilizationRatioBips = (borrowBalanceUsdc * 10000) / borrowLimitUsdc;
    }
    return
      AdapterPosition({
        supplyBalanceUsdc: supplyBalanceUsdc,
  ...
  function assetsTokensAddresses() public view returns (address[] memory) {
    ...
    for (uint256 assetIdx = 0; assetIdx < numberOfAssets; assetIdx++) {
      address assetAddress = _assetsAddresses[assetIdx];
      _tokensAddresses[assetIdx] = assetUnderlyingTokenAddress(assetAddress);
    }
  ...
  fallback() external {
    for (uint256 i = 0; i < _extensionsAddresses.length; i++) {
      ...
      assembly {
        ...
        let success := staticcall(gas(), _target, 0, calldatasize(), 0, 0)
