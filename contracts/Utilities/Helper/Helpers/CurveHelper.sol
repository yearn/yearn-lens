...
contract CurveHelper {
  ...
  function gaugesAddressesByPoolsAddressesSets(address[] memory _poolsAddresses)
    public
    view
    returns (address[][] memory)
  {
    ...
    for (uint256 poolIdx = 0; poolIdx < _poolsAddresses.length; poolIdx++) {
      ...
      address[] memory _poolGaugesAddresses = gaugesAddressesFromPoolAddress(
        poolAddress
      );
    ...
    returns (LpMetadata memory)
  {
    ...
    uint256 poolPrice = registry().get_virtual_price_from_lp_token(lpAddress);
