...
contract StrategiesHelper {
    ...
    returns (address[] memory)
  {
    ...
    address[] memory _strategiesAddresses = new address[](numberOfStrategies);
    for (
  ...
  function assetsStrategiesAddressesByFilter(string[][] memory filter)
    ...
    returns (address[] memory)
  {
    address[] memory _assetsStrategiesAddresses = assetsStrategiesAddresses();
    return
      assetsStrategiesAddressesByFilter(_assetsStrategiesAddresses, filter);
  }
    ...
    returns (address[] memory)
  {
    ...
    address[][] memory _strategiesForAssets = new address[][](numberOfAssets);
    for (uint256 assetIdx = 0; assetIdx < numberOfAssets; assetIdx++) {
      address assetAddress = _assetsAddresses[assetIdx];

      address[] memory _assetStrategiessAddresses = assetStrategiesAddresses(
        assetAddress
      );
  ...
  function assetsStrategiesByFilter(
    ...
    return
      strategies(assetsStrategiesAddressesByFilter(_assetsAddresses, _filter));
  }
  ...
  function strategyPassesFilter(
    ...
    for (
      ...
      if (operandIsOperator) {
        ...
        bytes memory data;
        if (operandIsKey) {
          (, bytes memory matchBytes) = address(strategyAddress).staticcall(
